...
export default class FooterPlayerTrack extends Component {
  constructor(
    private data: Song,
    private user: User,
  ) {
  ...
  override getTemplate(): string {
    ...
    <div class="player__track-name flex">
      <img class="player__track__img" src="${this.data.image}" alt="${this.data.artist.name}">
      <div class="player__track-name__content">
        <div class="flex player__name__header">
          <h3 class="player__track__h3">${this.data.name}</h3>
          <button class="player__btn__like ${this.data.likes.find((user) => user.id === this.user.id) ? "like-btn--active" : ""}"">
        ...
        <p class="player__track__author">${this.data.artist.name}</p>
      </div>
    </div>`;
  }
