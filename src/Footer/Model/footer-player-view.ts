...
export class PlayerView {
  ...
  update(state: PlayerState) {
    ...
    const progressBar = document.getElementById("range-play");
    const currentTime = document.querySelector(".player__time-start");
    const duration = document.querySelector(".player__time-end");

    if (trackName && trackAlbum) {
      trackName.innerHTML = state.currentTrack
        ? `${state.currentTrack.artist.name}`
        : "Трек не выбран";
      trackAlbum.innerHTML = state.currentTrack
    ...
    if (progressBar) {
    //   progressBar.innerHTML = `${state.currentTime}`;
    //   progressBar
    // console.log(progressBar);
    }

    if (currentTime) {
      currentTime.innerHTML = `${formatDuration(state.currentTime)}`
    }
  ...
  setupEventListeners() {
    const playBtn = document.querySelector(".player__play-btn");
    // const volumeSlider = document.getElementById('range-value')
